...
local function generate(p, opt)
	...
	local base = lush(function(injected_functions)
		...
		return {
			...
			NonText         { fg = p1.bg.da(opt.darken_non_text or 15) }, -- '@' at the end of the window, characters from 'showbreak' and other characters that do not really exist in the text (e.g., ">" displayed when a double-wide character doesn't fit at the end of the line). See also |hl-EndOfBuffer|.
			SpecialKey      { fg = p1.bg.da(opt.darken_whitespace or 40), gui = "italic" }, -- Unprintable characters: text displayed differently from what it really is.	But not 'listchars' whitespace. |hl-Whitespace|
